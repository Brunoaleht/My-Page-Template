...
import pageFake from './datas/page1Data.json';
...
describe('map-sections', () => {
  ...
  it('should map sections with data', () => {
    const data = mapSections(pageFake.data[0].attributes.sections);
  ...
  it('should map image-grid with data', () => {
    ...
    expect(pagedata.background).toBe(false);
    expect(pagedata.component).toBe('sections.section-grid-image');
    expect(pagedata.title).toBe('Gallery');
    expect(pagedata.description).toBe('Distinctio veritatis');
    expect(pagedata.sectionId).toBe('gallery');
    expect(pagedata.grid[0].altText).toBe('abc');
    expect(pagedata.grid[0].imgUrl).toBe('13a.jpg');
  });
});
